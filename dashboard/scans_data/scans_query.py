...
def all_scanners(project_id):
    scanners = []
    try:
        all_sast_scan = (StaticScansDb.objects.filter(project__uu_id=project_id)
                         .values_list('scanner',flat=True).distinct())
        if all_sast_scan.exists():
            sast_scanners = list(all_sast_scan)
            scanners.extend(sast_scanners)
    except Exception as e:
        #
        all_sast_scan = []
    try:
        all_cloud_scan = (CloudScansDb.objects.filter(project__uu_id=project_id)
                          .values_list('scanner',flat=True).distinct())
        if all_cloud_scan.exists():
            cloud_scanners = list(all_cloud_scan)
            scanners.extend(cloud_scanners)
    except Exception as e:
        #
        all_cloud_scan = []
    try:
        all_dast_scan = (WebScansDb.objects.filter(project__uu_id=project_id)
                          .values_list('scanner', flat=True).distinct())
        if all_dast_scan.exists():
            dast_scanners = list(all_dast_scan)
            scanners.extend(dast_scanners)
    except Exception as e:
        #
        all_dast_scan = []

    try:
        all_net_scan = (NetworkScanDb.objects.filter(project__uu_id=project_id)
                         .values_list('scanner', flat=True).distinct())
        if all_net_scan.exists():
            net_scanners = list(all_net_scan)
            scanners.extend(net_scanners)
    except Exception as e:
        #
        all_net_scan = []
    return scanners

def last_scan_date(project_id):

    last_sast_scan = StaticScansDb.objects.filter(project__uu_id=project_id).order_by('-created_time').first()
    last_cloud_scan = CloudScansDb.objects.filter(project__uu_id=project_id).order_by('-created_time').first()
    last_dast_scan = WebScansDb.objects.filter(project__uu_id=project_id).order_by('-created_time').first()
    last_net_scan = NetworkScanDb.objects.filter(project__uu_id=project_id).order_by('-created_time').first()

    last_scan_dates = [last_sast_scan, last_cloud_scan, last_dast_scan, last_net_scan]
    last_scan_dates = [scan.created_time for scan in last_scan_dates if scan]

    if last_scan_dates:
        return max(last_scan_dates)
    else:
        return ""